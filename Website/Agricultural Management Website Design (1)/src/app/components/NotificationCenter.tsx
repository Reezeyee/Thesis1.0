 ...
 * handed back via [onRevealed] so the caller can show it in the TempPasswordModal (copy / SMS).
 */
async function approvePasswordResetRequest(
  item: PendingReportItem,
  workers: WorkerRecord[],
  updateState: (updater: (prev: any) => any) => Promise<void> | void,
  onRevealed: (reveal: TempPasswordReveal) => void
) {
...
export function GlobalNotificationBanner({
  ...
  if (!latestPendingReport) {
    return <TempPasswordModal data={passwordReveal} onClose={() => setPasswordReveal(null)} />;
  }
